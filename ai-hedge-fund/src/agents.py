...
from tools import calculate_bollinger_bands, calculate_intrinsic_value, calculate_macd, calculate_obv, calculate_rsi, get_cash_flow_statements, get_financial_metrics, get_insider_trades, get_market_data, get_prices, prices_to_df
...
def fundamentals_agent(state: AgentState):
    ...
    cash_flow_statement = data["cash_flow_statements"]
    ...
    if metrics["profit_margins"] > 0.20:  # Healthy profit margins
        profitability_score += 1
    if metrics["operating_margins"] > 0.15:  # Strong operating efficiency
        profitability_score += 1
        
    signals.append('bullish' if profitability_score >= 2 else 'bearish' if profitability_score == 0 else 'neutral')
    reasoning["Profitability"] = {
        "signal": signals[0],
        "details": f"ROE: {metrics['return_on_equity']:.2%}, Net Margin: {metrics['profit_margins']:.2%}, Op Margin: {metrics['operating_margins']:.2%}"
    }
    ...
    # Compare intrinsic value (in billions) to market cap (in billions)
    if market_cap < intrinsic_value:
        signals.append('bullish')
    else:
        signals.append('bearish')

    # Display results in billions
    reasoning["Intrinsic_Value"] = {
        "signal": signals[4],
        "details": f"Intrinsic Value: ${intrinsic_value:,.2f}B, Market Cap: ${market_cap:,.2f}B"
    }
...
def sentiment_agent(state: AgentState):
    ...
    template = ChatPromptTemplate.from_messages(
        [
            ...
            (
                ...
                Only include ONE OVERALL sentiment based on the insider_trades, not one sentiment per trade and reasoning.
                Output ONLY a JSON object with the following keys:
                "sentiment": <bullish | bearish | neutral>,
                "reasoning": <concise explanation of the decision>
                YOU WILL NOT ADD MARKDOWN in the JSON output !
                """
            ),
        ]
    )
    ...
    try:
        ...
        if json_match:
            message_content = json.loads(json_match.group(0))
        else:
...
def risk_management_agent(state: AgentState):
    ...
    # Extract the JSON content, safely
    try:
        json_match = re.search(r"\{.*\}", result.content, re.DOTALL)
        if json_match:
            message_content = json.loads(json_match.group(0))
            ...
            message_content = {
                ...
                "risk_score": 10,
                "trading_action": "hold",
                "reasoning": "Unable to parse JSON output of risk analysis"
            }
    except json.JSONDecodeError:
        message_content = {
            "max_position_size": 0,
    ...
    try:
        ...
        if json_match:
            ...
            message_content = {
                "action": "hold",
                "quantity": 0,
                "reasoning": "Unable to parse JSON output of portfolio management decision"
            }
    except json.JSONDecodeError:
        message_content = {
            "action": "hold",
